...
void MEProcess::CrossSection(std::string result_path)
{
  SHERPA::Matrix_Element_Handler* me_handler = p_gen->GetInitHandler()->GetMatrixElementHandler();
  ...
  outfile.open("xsec.out");

  for(auto& p_proc: procs) {
    p_proc->Integrator()->ReadResults();
    std::string fname(p_proc->Name());
    std::cout << "Process " << p_proc->Name()
              << "read from directory " << result_path+"/"+p_proc->Generator()->Name()+"/XS_"+fname
              << "  with xs = " << p_proc->Integrator()->TotalXS()*ATOOLS::rpa->Picobarn() << std::endl;
    outfile << p_proc->Name() << '\n'
            << p_proc->Integrator()->TotalXS()*ATOOLS::rpa->Picobarn() << "\n\n";
