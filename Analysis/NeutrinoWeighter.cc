...
void NeutrinoWeighter::Reset(){  
  ...
  m_top   = FourMomentum(0,0,0,0);
  m_tbar  = FourMomentum(0,0,0,0);
  m_ttbar = FourMomentum(0,0,0,0);
  m_nu    = FourMomentum(0,0,0,0);
  m_nubar = FourMomentum(0,0,0,0);
  m_b     = FourMomentum(0,0,0,0);
  m_bbar  = FourMomentum(0,0,0,0);
  m_weight_max = -99.;
...
void NeutrinoWeighter::RecalculateEtas(double pos_lep_eta, double neg_lep_eta){
  ...
  for (int i = 0; i < 200; ++i){
    ...
    std::normal_distribution<double> dist(mean_nu, 1.14);
    double eta_nu = dist(generator);
    ...
    std::normal_distribution<double> dist2(mean_nubar, 1.14);
    double eta_nubar = dist2(generator);
...
NeutrinoWeighter::NeutrinoWeighter( int setting, int event_number ){
  ...
  if(setting ==2){
    ...
    m_do_crystalball_smearing = false;
  }
...
double NeutrinoWeighter::Reconstruct(FourMomentum lepton_pos, FourMomentum lepton_neg, FourMomentum jet_1, FourMomentum jet_2, double met_ex, double met_ey, double met_phi){
   
  //m_mt2 = mt2(jet_1, jet_2, leptons[0], leptons[1], m_met_et, 30);
  double width_1, width_2;
  if (jet_1.pt() < 40.0) width_1 = 0.13;
  else if (jet_1.pt() < 50.0) width_1 = 0.14;
  else if (jet_1.pt() < 60.0) width_1 = 0.12;
  else if (jet_1.pt() < 70.0) width_1 = 0.11;
  ...
  else if (jet_2.pt() < 70.0) width_2 = 0.11;
  ...
  for(size_t mtop_counter = 0; mtop_counter < m_top_smears.size(); ++mtop_counter){
	...
	for(size_t mWn_counter = 0; mWn_counter < m_W_smears.size(); ++mWn_counter){
	  ...
	  for (int smears = 0; smears < 5; ++smears){
	    FourMomentum jet_1_smeared, jet_2_smeared;
...
//	      scale_2 = GetCrystalBallWeight(jet_2.pt());
	      ...
	      std::default_random_engine generator;
	      std::normal_distribution<double> dists1(jet_1.pt(), width_1*jet_1.pt());
	      std::normal_distribution<double> dists2(jet_2.pt(), width_2*jet_2.pt());
	      scale_1 = dists1(generator)/jet_1.pt();
	      scale_2 = dists2(generator)/jet_2.pt();
	    }
	    ...
	    if (jet_1_smeared.mass() > 0.0 && jet_2_smeared.mass() > 0.0){	
...
float NeutrinoWeighter::GetCrystalBallWeight(float jet_pt) {
  ...
  if (jet_pt > 25. && jet_pt < 50.){
    scale = 1 + pt_25_50.GetRandom();
  }
  if (jet_pt > 50. && jet_pt < 100.){
...
void NeutrinoWeighter::calculateWeight(FourMomentum lepton_pos, FourMomentum lepton_neg, FourMomentum b1, FourMomentum b2, double met_ex, double met_ey, double met_phi, double mtop, double mtbar, double mWp, double mWn) {

  double weight = -1.;
  FourMomentum top, tbar, ttbar, Wp, Wn;
  ...
  for (unsigned int nu_eta_index = 0; nu_eta_index < m_nu_eta.size(); ++nu_eta_index){    
    ...
    std::vector<FourMomentum> solution_1 = solveForNeutrinoEta(&lepton_pos, &b1, nu_eta_index, 1, mtop, mWp);
    ...
    for (unsigned int nubar_eta_index = 0; nubar_eta_index < m_nubar_eta.size(); ++nubar_eta_index){
      ...
      std::vector<FourMomentum> solution_2 = solveForNeutrinoEta(&lepton_neg, &b2, nubar_eta_index, -1, mtbar, mWn);
...
//double NeutrinoWeighter::neutrino_weight(FourMomentum neutrino1, FourMomentum neutrino2, double met_ex, double met_ey, double met_phi){
double NeutrinoWeighter::neutrino_weight(FourMomentum neutrino1, FourMomentum neutrino2, double met_ex, double met_ey, double met_phi, FourMomentum lep_pos, FourMomentum lep_neg, FourMomentum b1, FourMomentum b2, double mtop, double mtbar, double mWp, double mWn){

  double dx = met_ex - neutrino1.px() - neutrino2.px();
  double dy = met_ey - neutrino1.py() - neutrino2.py();
  double dphi = 1.;
  if(met_phi > -99.){
    FourMomentum nunubar = neutrino1 + neutrino2;
    dphi = met_phi - nunubar.phi();
  }
  FourMomentum Wp   = neutrino1 + lep_pos;
  FourMomentum Wn   = neutrino2 + lep_neg;
  FourMomentum top  = neutrino1 + lep_pos + b1;
  FourMomentum tbar = neutrino2 + lep_neg + b2;
...
//	<< "\nb2: " << b2 << std::endl;
  ...
  double dmWp = mWp - Wp.mass(); 
  double dmWn = mWn - Wn.mass(); 
  double dmtop = mtop - top.mass(); 
  double dmtbar = mtbar - tbar.mass();
  //std::cout << "dmtop = " << dmtop << ", dmtbar = " << dmtbar << std::endl;
  //std::cout << "dmWp = " << dmWp << ", dmWn = " << dmWn << std::endl;
  ...
  double m_sigma_mWp     = 11.;//0.015
  double m_sigma_mWn     = 11.;//0.015
  double m_sigma_mtop    = 21.; //0.61
  double m_sigma_mtbar   = 21.; //0.61
  ...
  double exp_all = 1;
  if(m_include_x){
    //std::cout << "exp_x = " << exp_x << std::endl;
    exp_all *= exp_x;
  }
  if(m_include_y){
    //std::cout << "exp_y = " << exp_y << std::endl;
    exp_all *= exp_y;
  }
  if(m_include_phi){
    //std::cout << "exp_phi = " << exp_phi << std::endl;
    exp_all *= exp_phi;
  }
  if(m_include_mWp){
    //std::cout << "exp_mWp = " << exp_mWp << std::endl;
    exp_all *= exp_mWp;
  }
  if(m_include_mWn){
    //std::cout << "exp_mWn = " << exp_mWn << std::endl;
    exp_all *= exp_mWn;
  }
  if(m_include_mtop){
    //std::cout << "exp_mtop = " << exp_mtop << std::endl;
    exp_all *= exp_mtop;
  }
  if(m_include_mtbar){
    //std::cout << "exp_mtbar = " << exp_mtbar << std::endl;
    exp_all *= exp_mtbar;
  }
...
std::vector<FourMomentum> NeutrinoWeighter::solveForNeutrinoEta(FourMomentum* lepton, FourMomentum* bJet, int index, int index_type, double mtop, double mW) {
  ...
  if (det == 0.) {
    ...
    FourMomentum a1(sqrt(pT2_1 + pz1 * pz1), px1, py1, pz1);
  ...
  if(det > 0){
    ...
    FourMomentum a1(sqrt(pT2_1 + pz1 * pz1), px1, py1, pz1);
    FourMomentum a2(sqrt(pT2_2 + pz2 * pz2), px2, py2, pz2);

    if (!std::isnan(a1.E()) && !std::isnan(a2.E())){
      sol.push_back(a1);
      sol.push_back(a2);
    }
    return sol;
...
/*double TopDileptonEventSaver::mt2(FourMomentum jet_1, FourMomentum jet_2, FourMomentum lep_1, FourMomentum lep_2, double met, int n_points){
 
  double mt2_result = 999999.;#

  FourMomentum visible_1 = jet_1 + lep_1;
  FourMomentum visible_2 = jet_2 + lep_2;

  double energy_1 = sqrt(pow(visible_1.mass(), 2) + pow(visible_1.pt(), 2));
  double energy_2 = sqrt(pow(visible_2.mass(), 2) + pow(visible_2.pt(), 2));
  ...
  for( int i = 0; i < n_points; ++i){
    ...
    double result_1 = sqrt( pow(visible_1.mass(), 2) + 2*(energy_1*fabs(daughter_1) - (visible_1.pt()*daughter_1)));
    double result_2 = sqrt( pow(visible_1.mass(), 2) + 2*(energy_1*fabs(daughter_1) - (visible_1.pt()*daughter_1)));
