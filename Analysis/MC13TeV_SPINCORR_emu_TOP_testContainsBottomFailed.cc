...
// Modified containsBottom() routine
bool containsBottom(const Jet& jet) {

  foreach (const Particle& p, jet.particles()) {
    const PdgId pid = p.pdgId();
    std::cout << "Particle " << pid << ": ";
    if (abs(pid) == 5) { std::cout << "b." << std::endl; return true; }
    if (PID::isHadron(pid) && PID::hasBottom(pid)) { std::cout << "B-hadron." << std::endl; return true; }
    std::cout << std::endl;
  }
  ...
  class MC13TeV_SPINCORR_emu_TOP : public Analysis {

#include "NLOHisto1D.cc"

  private:

    NLOHisto1DPtr _h_dphil, _h_mll, _h_el_pt, _h_el_eta, _h_el_phi, _h_mu_pt, _h_mu_eta, _h_mu_phi, _h_njets, _h_nbjets, _h_pTb1, _h_pTb2, _h_pTj1, _h_pTj2, _h_DRj, _h_dphij, _h_mjj, _h_ht, _h_ht2, _h_meff, _h_MT, _h_MT0j, _h_MT1j, _h_MT2j, _h_MT3j, _h_MT4j, _h_MTooj, _h_met, _h_topPt, _h_topPtLead, _h_topPtSubLead, _h_tMass, _h_ttbarMass, _h_v_topPt, _h_v_topPtLead, _h_v_topPtSubLead, _h_v_tMass, _h_v_ttbarMass, _h_v_ttbarMassATLAS, _h_v_ttbarMassATLAS2, _h_selected_events, _h_xs, _h_xscut, _h_eff, _h_dR_el_j_incl, _h_dR_mu_j_incl, _h_nbjets_incl_dR0p1, _h_nbjets_incl_dR0p2, _h_nbjets_incl_dR0p3;
...
//	std::make_pair(0.,_EcmsTag_)};
  ...
  public:
    ...
    /// Constructor
    /// Constructor
    MC13TeV_SPINCORR_emu_TOP()
      : Analysis("MC13TeV_SPINCORR_emu_TOP")
    {    }
...
//      addProjection(wfek, "WFinder_ek");
      ...
      addProjection(FastJets(vfs, FastJets::ANTIKT, 0.3), "Jets0p3");
      addProjection(FastJets(vfs, FastJets::ANTIKT, 0.2), "Jets0p2");
      addProjection(FastJets(vfs, FastJets::ANTIKT, 0.1), "Jets0p1");
...
//	50.*PI/60.,55.*PI/60.,PI};
	...
	PI};
      ...
      _h_nbjets_incl_dR0p3 = bookNLOHisto1D("nbjets0p3", 6., -0.5, 5.5);
      _h_nbjets_incl_dR0p2 = bookNLOHisto1D("nbjets0p2", 6., -0.5, 5.5);
      _h_nbjets_incl_dR0p1 = bookNLOHisto1D("nbjets0p1", 6., -0.5, 5.5);
      ...
      _h_el_eta = bookNLOHisto1D("el_eta", 20, -2.5, 2.5);
      _h_mu_eta = bookNLOHisto1D("mu_eta", 20, -2.5, 2.5);
      _h_el_pt = bookNLOHisto1D("el_pt", 100, 0, 500);
      _h_mu_pt = bookNLOHisto1D("mu_pt", 100, 0, 500);
      _h_el_phi = bookNLOHisto1D("el_phi", 20, 0, 2*M_PI);
      _h_mu_phi = bookNLOHisto1D("mu_phi", 20, 0, 2*M_PI);
      _h_dR_el_j_incl = bookNLOHisto1D("dR_el_j_incl", 50, 0, 5.);
      _h_dR_mu_j_incl = bookNLOHisto1D("dR_mu_j_incl", 50, 0, 5.);
      _h_selected_events = bookNLOHisto1D("h_selected_events", 11, 0.0, 11.0);
    ...
    void analyze(const Event& event) {
      ...
      const FastJets& jetpro0p3 = applyProjection<FastJets>(event, "Jets0p3");
      const Jets alljets0p3 = jetpro0p3.jetsByPt();
      const FastJets& jetpro0p2 = applyProjection<FastJets>(event, "Jets0p2");
      const Jets alljets0p2 = jetpro0p2.jetsByPt();
      const FastJets& jetpro0p1 = applyProjection<FastJets>(event, "Jets0p1");
      const Jets alljets0p1 = jetpro0p1.jetsByPt();

      foreach (const Jet& jet, alljets) {
        if (jet.momentum().pT() > 25*GeV && fabs(jet.momentum().eta()) < 2.5) {
	  goodJets.push_back(jet);
	  if ( containsBottom(jet) ) bJets.push_back(jet);
        }
      }
      foreach (const Jet& jet, alljets0p3) {
        if (jet.momentum().pT() > 25*GeV && fabs(jet.momentum().eta()) < 2.5) {
	  if ( containsBottom(jet) ) bJets0p3.push_back(jet);
        }
      }
      foreach (const Jet& jet, alljets0p2) {
        if (jet.momentum().pT() > 25*GeV && fabs(jet.momentum().eta()) < 2.5) {
	  if ( containsBottom(jet) ) bJets0p2.push_back(jet);
        }
      }
      foreach (const Jet& jet, alljets0p1) {
        if (jet.momentum().pT() > 25*GeV && fabs(jet.momentum().eta()) < 2.5) {
	  if ( containsBottom(jet) ) bJets0p1.push_back(jet);
        }
      }

      _h_nbjets_incl_dR0p3->fill(bJets0p3.size(), weight);
      _h_nbjets_incl_dR0p2->fill(bJets0p2.size(), weight);
      _h_nbjets_incl_dR0p1->fill(bJets0p1.size(), weight);
	...
	foreach (const Jet& jet, goodJets) {
          _h_dR_el_j_incl->fill( deltaR(elec, jet), weight);
	...
	bool overlaps = false;
        foreach (const Jet& jet, goodJets) {
          _h_dR_mu_j_incl->fill( deltaR(muon, jet), weight);
	...
	if (elecs[0].pT() < 27*GeV && muons[0].pT() < 27*GeV) vetoEvent;
	  ...
	  _h_selected_events->fill(7.5, weight);
...
//	    assert(elecs[1].momentum()==wfel.constituentLepton().momentum());
	  ...
	  _h_selected_events->fill(10.5, weight);
#ifdef _SHOW_MASS_CUT_MSGS_
    ...
    //        std::cout << jet.momentum() << ", m2 = " << jet.mass2() << std::endl;
	  ...
	  _h_mll->fill( mll, weight);
          ...
          _h_el_pt->fill( elecs[0].pT(), weight );
          _h_el_eta->fill( elecs[0].eta(), weight );
          _h_el_phi->fill( elecs[0].phi(), weight );
          _h_mu_pt->fill( muons[0].pT(), weight );
          _h_mu_eta->fill( muons[0].eta(), weight );
          _h_mu_phi->fill( muons[0].phi(), weight );
...
//	    std::cout << "vw_antitop mom. = " << tbar << std::endl;
	...
	v_cutpTlb_histpTlb[i]->finalize();  scale(v_cutpTlb_histpTlb[i], fctr);
      ...
      _h_nbjets_incl_dR0p3->finalize();          scale(_h_nbjets_incl_dR0p3, fctr);
      _h_nbjets_incl_dR0p2->finalize();          scale(_h_nbjets_incl_dR0p2, fctr);
      _h_nbjets_incl_dR0p1->finalize();          scale(_h_nbjets_incl_dR0p1, fctr);
      ...
      _h_el_pt->finalize();           scale(_h_el_pt, fctr);
      _h_mu_pt->finalize();           scale(_h_mu_pt, fctr);
      _h_el_eta->finalize();           scale(_h_el_eta, fctr);
      _h_mu_eta->finalize();           scale(_h_mu_eta, fctr);
      _h_el_phi->finalize();           scale(_h_el_phi, fctr);
      _h_mu_phi->finalize();           scale(_h_mu_phi, fctr);
      _h_dR_el_j_incl->finalize();    scale(_h_dR_el_j_incl, fctr);
      _h_dR_mu_j_incl->finalize();    scale(_h_dR_mu_j_incl, fctr);
  ...
  DECLARE_RIVET_PLUGIN(MC13TeV_SPINCORR_emu_TOP);
